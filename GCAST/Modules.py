...
class Module(nn.Module):
    ...
    def forward(self, x, adj):
        # adj_mask, x_mask, (mask_nodes, keep_nodes) = self.encoding_mask_noise(adj, x, self._mask_rate)  #
        mu, logvar, feat_x = self.encode(x, adj)
        gnn_z = self.reparameterize(mu, logvar)
        z = torch.cat((feat_x, gnn_z), 1)
        # print("z.shape:", z.shape)
        # z.shape: torch.Size([4226, 32])
        ...
        # q = (q.t() / torch.sum(q, 1)).t() # q.t()指的是转置操作
    ...
    #     use_adj = adj.clone()
    #
    #     if use_adj.is_sparse:
    #         use_adj = use_adj.to_dense()
    #
...
class DMGCN(nn.Module):
    def __init__(
        ...
        self.dec_cluster_n = dec_clsuter_n
        self.latent_dim = self.gcn_hidden2 + self.feat_hidden2
        # self.latent_dim = self.gcn_hidden1 + self.feat_hidden1
        # self.latent_dim = self.gcn_hidden1 * 2
        self.layer_num = layer_num
        ...
        if self.layer_num == 1:
            self.gc1 = GraphConvolution(self.feat_hidden2, self.gcn_hidden2, self.p_drop, act=F.relu)
            # self.gc1.add_module("GCN_encoder", GraphConvolution(self.feat_hidden1, self.gcn_hidden1, self.p_drop, act=F.relu))
            # self.gc1.add_module("MLP_encoder", full_block(self.gcn_hidden1, self.gcn_hidden1, 0) )
        elif self.layer_num > 1:
            layer_dim = [self.feat_hidden2] + [self.gcn_hidden2]*self.layer_num
            self.gc1 = MultiGraphConvolution(
                layer_dims=layer_dim,
                dropout=self.p_drop,
                activations=[F.relu] * self.layer_num ,
            )
            # self.gc1.add_module("GCN_encoder",  multiGCN)
            # self.gc1.add_module("MLP_encoder", full_block(self.gcn_hidden1, self.gcn_hidden1, 0))
        # self.mlp = full_block(self.gcn_hidden1, self.gcn_hidden1, 0)
        # self.gc2 = GraphConvolution(self.gcn_hidden1, self.gcn_hidden2, self.p_drop, act=lambda x: x)
        # self.gc3 = GraphConvolution(self.gcn_hidden1, self.gcn_hidden2, self.p_drop, act=lambda x: x)
        ...
        # DEC cluster layer
        # self.cluster_layer = Parameter(torch.Tensor(self.dec_cluster_n, self.gcn_hidden2 + self.feat_hidden2))
        # self.cluster_layer = Parameter(torch.Tensor(self.dec_cluster_n, self.gcn_hidden1 + self.feat_hidden1))
    ...
    def encode(self, x, adj):
        feat_x = self.encoder(x)
        feat1 = self.gc1(feat_x, adj)
        # feat2 = self.mlp(feat1)
        feat  = torch.cat((feat1, feat_x ), 1)
        # return self.gc2(hidden1, adj), self.gc3(hidden1, adj), feat_x
        return feat
    ...
    def forward(self, x, adj):
        # adj_mask, x_mask, (mask_nodes, keep_nodes) = self.encoding_mask_noise(adj, x, self._mask_rate)  #
        feat = self.encode(x, adj)
        # gnn_z = self.reparameterize(mu, logvar)
        # z = torch.cat((feat_x, gnn_z), 1)
        # print("z.shape:", z.shape)
        # z.shape: torch.Size([4226, 32])
        de_feat = self.decoder(feat, adj)
